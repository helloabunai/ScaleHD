...
class ScaleHD:
	def __init__(self):
		...
		self.parser.add_argument('-t', '--threads', help='Thread utilisation. Typically only alters third party alignment performance. Default: system max.', type=int, choices=range(1, THREADS+1), default=THREADS)
		...
		##
		## Check we're on python 3.7.6 or later
	...
	def sequence_workflow(self):
		...
		for i in range(len(data_pairs)):
			for seqpair_lbl, seqpair_dat in data_pairs[i].items():
				################################################
